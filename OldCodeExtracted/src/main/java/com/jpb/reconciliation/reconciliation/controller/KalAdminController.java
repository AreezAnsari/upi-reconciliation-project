...
import com.jpb.reconciliation.reconciliation.service.KalAdminService;

import io.swagger.v3.oas.annotations.Operation;

@RestController
@RequestMapping("/api/kalinfotech/auth")
public class KalAdminController {

    private Logger logger = LoggerFactory.getLogger(KalAdminController.class);

    @Autowired
    private KalAdminService kalAdminService;
    ...
    public ResponseEntity<RestWithStatusList> register(@RequestBody KalAdminDto dto) {
        logger.info("Register request received for username: {}", dto.getUsername());
        return kalAdminService.register(dto);
    }
}
