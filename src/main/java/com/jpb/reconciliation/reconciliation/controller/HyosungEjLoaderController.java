...
public class HyosungEjLoaderController {
    ...
    private final HyosungEjFileLoadService ejFileLoadService;

    @PostMapping("/load")
    public ResponseEntity<RestWithStatusList> loadEjFiles(@RequestBody Map<String, Object> request) {

        HyosungEjFileLoadService.EjLoadResult result = ejFileLoadService.loadEjFiles(request);

        if (result.errorMessage != null) {
            LOG.error("Hyosung EJ load failed: {}", result.errorMessage);
        ...
        LOG.info("Hyosung EJ load complete. batchId={}, status={}, inserted={}",
