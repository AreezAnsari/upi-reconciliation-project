...
/**
 * Controller for Field Format master APIs.
 * Split from ReconFieldMastController as per Sir's Point 3 — separate controller per resource.
 */
@RestController
@RequestMapping(path = "/api/v2/field/field-formats")
@RequiredArgsConstructor
@Tag(name = "Field Format Master", description = "APIs for fetching field formats")
public class ReconFieldFormatMastController {

    private final ReconFieldMastService reconFieldMastService;

    // ─────────────────────────────────────────────────────────────────────────
    // GET  /api/v2/field/field-formats
    // Returns all rows from recon_field_format_mast
    // ─────────────────────────────────────────────────────────────────────────

    @GetMapping(produces = CommonConstants.APPLICATION_JSON)
    @Operation(summary = "Get all field formats", description = "Returns all records from recon_field_format_mast")
    public ResponseEntity<RestWithMapStatusList> getAllFieldFormats() {
        return reconFieldMastService.getAllFieldFormats();
    }
}
