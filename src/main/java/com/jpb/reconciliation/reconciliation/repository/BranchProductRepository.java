...
public interface BranchProductRepository extends JpaRepository<BranchBankProduct, Long> {
    ...
    /**
     * Fetch purchased product names by branchCode string.
     * Joins BRANCH_BANK_PRODUCT → BRANCH_BANK on branchId to resolve the code.
     * Used by RecRoleController which receives branchCode from the auth store.
     */
    @Query("SELECT p.productName FROM BranchBankProduct p " +
           "JOIN BranchBank b ON b.branchId = p.branchId " +
           "WHERE b.branchCode = :branchCode")
    List<String> findProductNamesByBranchCode(@Param("branchCode") String branchCode);
