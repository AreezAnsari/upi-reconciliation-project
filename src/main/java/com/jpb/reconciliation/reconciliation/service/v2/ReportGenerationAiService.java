...
import com.jpb.reconciliation.reconciliation.service.ReportGenerationServiceImpl;
...
@Service
public class ReportGenerationAiService {
	...
	public ReportGenerationAiService(DataSource dataSource) {
	...
	public ResponseEntity<ResponseDto> generateReport(Optional<ReconTmpltFieldDtls> reconTemplateFileDetails, StringBuilder output,
			ReconBatchProcessEntity reconProcessManager, String dataCount, File file) throws JRException, IOException {

//		if (reconTemplateFileDetails.getFileUpdateFlag().equalsIgnoreCase("Y")) {
//			Boolean reportStatus = writeDataUpdationReport(reconTemplateFileDetails, reconProcessManager, file);
...
//		}
		...
		BigDecimal getTotalAmount = getTotalAmount(reconTemplateFileDetails.get().getTemplate().getStageTabName());
		...
		try {
			...
			String reportName = genertateReportName(reconTemplateFileDetails.get().getTemplate().getTemplateName(), file.getName().toString(),
					reconProcessManager);
			exportReportToTextFile(jasperPrint, reportName);
			File getFileName = new File(reportName);
//			newReport.setProcessId(reconTemplateFileDetails.getReconFileId());
			newReport.setTemplateId(reconTemplateFileDetails.get().getTemplate().getTemplateId());
			DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd-MM-yyyy");
			LocalDate localDate = LocalDate.parse(formattedDate, formatter);
			newReport.setReportDate(LocalDate.now());
			newReport.setFileName(file.getName().toString());
			newReport.setReportFileName(reconTemplateFileDetails.get().getTemplate().getTemplateName());
...
//	private Boolean writeDataUpdationReport(ReconFileDetailsMaster reconTemplateFileDetails,
...
//			newReportData.setTemplateId(reconTemplateFileDetails.ge)
