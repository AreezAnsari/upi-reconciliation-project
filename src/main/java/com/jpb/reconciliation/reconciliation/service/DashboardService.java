...
public class DashboardService {
    ...
    private UpiDashboardResponse buildDashboard(ReconProcessDefMaster def) {
        ...
        List<ReconBatchProcessEntity> reconRows =
                batchRepo.findLatestReconByProcessId(def.getReconProcessId(), ONE);
        ...
        double matchRate = totalTxns > 0
                ? Math.round((autoReconciled * 100.0 / totalTxns) * 10.0) / 10.0
                : 0.0;
        ...
        KpiSummary kpi = KpiSummary.builder()
                ...
                .forceMatchEligible(forceMatchEligible)
                .forceMatchNote(forceMatchNote)
                .pendingTtumApproval(0)      // TODO: wire from TTUM table
                .pendingTtumNote("Maker-checker pending")
                .settlementNet(BigDecimal.ZERO)      // TODO: wire from settlement table
                .settlementExpected(BigDecimal.ZERO)
                .settlementDelta(BigDecimal.ZERO)
                .fileWiseCount(fileWiseCount)
                .build();

        return UpiDashboardResponse.builder()
                .reconProcessId(def.getReconProcessId())
                .processName(def.getReconProcessName())
                .tranDate(LocalDate.now().minusDays(1))
                .kpiSummary(kpi)
                .build();
    }
    ...
    private FileCount buildFileCount(int slot, Long fileTypeProcessId, String dataTableName) {
        try {
            List<ReconBatchProcessEntity> rows =
                    batchRepo.findLatestExtractionByFileTypeProcessId(fileTypeProcessId, ONE);

            if (!rows.isEmpty()) {
                ReconBatchProcessEntity row = rows.get(0);
                return FileCount.builder()
                        .fileSlot(slot)
                        .fileTypeProcessId(fileTypeProcessId)
                        .dataTableName(dataTableName)
                        .fileName(row.getFileName())
                        .dataCount(parseCount(row.getDataCount()))
                        .extStatus(row.getExtractionStatus())
                        .build();
            }
        } catch (Exception e) {
            log.error("Error fetching extraction for slot={} fileTypeProcessId={} : {}",
                    slot, fileTypeProcessId, e.getMessage());
        }
